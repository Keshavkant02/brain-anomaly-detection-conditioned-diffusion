...
def main(args=None):
    ...
    if  os.path.isdir(os.path.join(Path(args.img_dir).parents[0],'mask')) and args.nomaskandseg !=1:
        ...
        mask_path = None
        mask_out = None # BUG FIX

    for i, file in tqdm(enumerate(os.listdir(args.img_dir))):
        
        # BUG FIX: Safely check conditions without throwing errors
        needs_processing = False
        if not os.path.isfile(os.path.join(dest_basepath, file)):
            needs_processing = True
        elif mask_out is not None and not os.path.isfile(os.path.join(mask_out, file.replace(args.modality,'_mask'))):
            needs_processing = True
        elif seg_out is not None and not os.path.isfile(os.path.join(seg_out, file.replace(args.modality,'_seg'))):
            needs_processing = True
