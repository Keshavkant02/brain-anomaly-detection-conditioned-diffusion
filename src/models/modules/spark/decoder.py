...
class DecoderConv3D(nn.Module):
    def __init__(self, cin, cout, double, heavy, cmid):
        ...
        self.up = nn.ConvTranspose3d(
            cin, cin,
            kernel_size=(4 if double else 2, 4 if double else 2, 4 if double else 2),
            stride=(2, 2, 2),
            padding=(1 if double else 0, 1 if double else 0, 1 if double else 0),
            bias=True
        )
        ls = [UNetBlock3D(cin, (cin if i != heavy[1]-1 else cout), cmid=cmid, last_act=i != heavy[1]-1)
...
class LightDecoder3D(nn.Module):
    def __init__(self, decoder_fea_dim, upsample_ratio, double=False, heavy=None, cmid=0, sbn=False):
        ...
        channels = [self.fea_dim // 2**i for i in range(n+1)]
        self.dec = nn.ModuleList([
            DecoderConv3D(cin, cout, double, heavy, cmid)
